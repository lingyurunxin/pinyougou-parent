...
public class ItemSearchServiceImpl implements ItemSearchService {
    ...
    private Map searchList(Map searchMap) {
        ...
        //1.2 按商品分类过滤
        if (!"".equals(searchMap.get("category").toString().trim())) {//如果用户选择了分类
        ...
        //1.3 按品牌过滤
        if (!"".equals(searchMap.get("brand").toString().trim())) {//如果用户选择了品牌
            FilterQuery filterQuery = new SimpleFilterQuery();
        ...
        //1.5按照价格过滤
        if (!"".equals(searchMap.get("price").toString().trim())) {
